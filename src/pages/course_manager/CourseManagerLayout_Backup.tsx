...
import {
  ...
  Bell, Menu, X, ChevronDown, LogOut, Settings, User, Clock, MessageSquare, Trophy, TrendingUp, Activity, BookOpen as GitMerge
} from 'lucide-react';
...
const cmMenuGroups: NavGroup[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { name: 'Performance Analysis', path: '/course-manager/performance', icon: <TrendingUp size={15} /> },
      { name: 'Completion Insights', path: '/course-manager/insights', icon: <Activity size={15} /> },
    ]
  },
  {
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        background: '#ffffff',
        borderRadius: '14px',
        boxShadow: '0 12px 40px rgba(200, 16, 46, 0.15), 0 4px 12px rgba(0,0,0,0.08)',
        ...
        return (
          <Link
            ...
            style={{
              ...
              background: isActive ? 'rgba(200, 16, 46, 0.05)' : 'transparent',
...
const NavButton: React.FC<NavButtonProps> = ({ group, activePath, isOpen, onClick }) => {
  const hasActive = group.isDirectLink 
    ? activePath.startsWith(group.path || '') 
    : group.items.some(i => activePath.startsWith(i.path));
    ...
    return (
      <Link
        ...
        style={{
          ...
          borderRadius: '12px',
          textDecoration: 'none',
          background: hasActive ? 'rgba(200, 16, 46, 0.05)' : 'transparent',
          color: hasActive ? brandPrimary : '#334155',
      ...
      >
        <span style={{ color: hasActive ? brandPrimary : '#64748b' }}>{group.icon}</span>
        {group.section}
      </Link>
    );
  }
  ...
  return (
    <button
      ...
      style={{
        ...
        borderRadius: '12px',
        border: 'none',
        background: isOpen || hasActive ? 'rgba(200, 16, 46, 0.05)' : 'transparent',
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', background: '#f8fafc', overflow: 'hidden', fontFamily: "'Inter', sans-serif" }}>
      ...
      <header style={{
        ...
        zIndex: 1000,
        flexShrink: 0,
        boxShadow: '0 1px 8px rgba(200, 16, 46, 0.04)',
        position: 'relative',
      }}>
        ...
        <Link to="/course-manager/dashboard" style={{ display: 'flex', alignItems: 'center', textDecoration: 'none', flexShrink: 0 }}>
          ...
          <span style={{ marginLeft: '12px', fontSize: '16px', fontWeight: 800, color: brandPrimary, letterSpacing: '-0.01em' }}>Manager</span>
        </Link>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexShrink: 0, marginLeft: '12px' }}>
          {(cmUser?.role === 'admin' || cmUser?.role === 'super_admin') && (
            <button 
              ...
              onMouseEnter={e => e.currentTarget.style.background = 'rgba(200, 16, 46, 0.02)'}
              onMouseLeave={e => e.currentTarget.style.background = '#fff'}
            >
              <GitMerge size={16} /> <span className="desktop-nav-text">Back to Admin</span>
            </button>
          )}

          <button
            style={{ background: 'none', border: 'none', cursor: 'pointer', color: brandPrimary, display: 'flex', padding: '8px', borderRadius: '10px', transition: '0.2s' }}
          ...
          <div ref={profileRef} style={{ position: 'relative' }}>
            ...
            >
              {cmUser?.profile_picture ? (
                <img src={cmUser.profile_picture} alt={cmUser.full_name} style={{ width: '36px', height: '36px', borderRadius: '10px', objectFit: 'cover' }} />
              ) : (
                <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: brandPrimary, color: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, fontSize: '15px' }}>
                  {initials}
                </div>
              )}
              ...
              {profileDropdownOpen && (
                <motion.div
                  ...
                  style={{
                    ...
                    background: '#ffffff',
                    borderRadius: '14px',
                    boxShadow: '0 8px 32px rgba(200, 16, 46, 0.12)',
                ...
                >
                  ...
                  <button onClick={handleLogout} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '10px', width: '100%', textAlign: 'left', color: '#ef4444', fontSize: '14px', fontWeight: 600, border: 'none', background: 'none', cursor: 'pointer', transition: '0.15s' }} onMouseEnter={e => (e.currentTarget.style.background = '#fff1f1')} onMouseLeave={e => (e.currentTarget.style.background = 'transparent')}>
          ...
          <button
            ...
            style={{ display: 'none', alignItems: 'center', background: 'none', border: 'none', cursor: 'pointer', color: '#334155', padding: '8px', borderRadius: '10px' }}
          >
            <Menu size={24} />
          </button>
        </div>
      ...
      <AnimatePresence>
        {mobileOpen && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 10000, display: 'flex' }}>
            ...
            <motion.div initial={{ x: '-100%' }} animate={{ x: 0 }} exit={{ x: '-100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} style={{ position: 'relative', width: '280px', background: '#fff', height: '100%', boxShadow: '4px 0 32px rgba(200, 16, 46, 0.15)', padding: '20px', display: 'flex', flexDirection: 'column' }}>
                  ...
                  {cmMenuGroups.map(group => (
                    <div key={group.section} style={{ marginBottom: '16px' }}>
                       ...
                       {group.isDirectLink && group.path ? (
                         <Link to={group.path} onClick={() => setMobileOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 0', color: location.pathname.startsWith(group.path) ? brandPrimary : '#334155', textDecoration: 'none', fontWeight: location.pathname.startsWith(group.path) ? 700 : 500, fontSize: '14px' }}>
                            <span style={{ color: location.pathname.startsWith(group.path) ? brandPrimary : '#64748b' }}>{group.icon}</span>
                            {group.section}
                         </Link>
                       ) : (
                         group.items.map(item => (
                           <Link key={item.path} to={item.path} onClick={() => setMobileOpen(false)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 0', color: location.pathname.startsWith(item.path) ? brandPrimary : '#334155', textDecoration: 'none', fontWeight: location.pathname.startsWith(item.path) ? 700 : 500, fontSize: '14px' }}>
                              <span style={{ color: location.pathname.startsWith(item.path) ? brandPrimary : '#64748b' }}>{item.icon}</span>
                              {item.name}
                           </Link>
                         ))
                       )}
